/**
 * Seed Volunteers and Teams Script
 * 
...
async function seedData() {
  try {
    ...
    // Dynamically import models using require to handle path aliases
    const modelsPath = resolve(process.cwd(), 'src/models');
    const UserModule = require(`${modelsPath}/User`);
    const VolunteerModule = require(`${modelsPath}/Volunteer`);
    const VolunteerTeamModule = require(`${modelsPath}/VolunteerTeam`);
    
    const User = UserModule.default || UserModule;
    const Volunteer = VolunteerModule.default || VolunteerModule;
    const VolunteerTeam = VolunteerTeamModule.default || VolunteerTeamModule;
    ...
    // Team definitions (will be created after volunteers)
    ...
    for (const volData of volunteerData) {
      ...
      console.log(`  Creating user for ${volData.name}:`, {
        firstName,
        lastName,
      ...
      const user = await User.create({
        ...
        name: volData.name || `${firstName} ${lastName}`.trim(),
        email: volData.email.toLowerCase(),
        phone: volData.phone || '',
      ...
      console.log(`  ✅ User created - Verified:`, {
        firstName: verifyUser?.firstName || '❌ MISSING',
        lastName: verifyUser?.lastName || '❌ MISSING',
        email: verifyUser?.email || '❌ MISSING',
        phone: verifyUser?.phone || '❌ MISSING'
      });
      ...
      createdVolunteers.push({ volunteer, teamIndex: volData.teamIndex, isLead: volData.isLead });
      console.log(`  ✅ Created volunteer: ${volData.name} (${(volunteer as any).volunteerId})`);
    }

    // Create teams with assigned leads and members
    console.log('\n👥 Creating teams with assigned volunteers...');
    const createdTeams = [];
    for (let i = 0; i < teamDefinitions.length; i++) {
      const teamDef = teamDefinitions[i];
      const teamVolunteers = createdVolunteers.filter(v => v.teamIndex === i);
      const lead = teamVolunteers.find(v => v.isLead);
      const members = teamVolunteers.map(v => (v.volunteer as any)._id.toString());

      if (lead && members.length > 0) {
        const leadId = (lead.volunteer as any)._id.toString();
        
        // Create team with lead and members
        ...
        console.log(`  ✅ Created team: ${team.name} (${team.teamId}) with ${members.length} members`);
      }
    }
    ...
    console.log(`   - All volunteers assigned to teams`);
    console.log('\n🔑 Default password for all volunteers: volunteer123');
    console.log('\n📧 You can login with any volunteer email and password: volunteer123');
