...
export default function WeatherClient({ initialMultiCityWeather, initialAlerts }: WeatherClientProps) {
  ...
  // Fetch full weather data for selected city on mount if available
  useEffect(() => {
    if (selectedCity && !hourlyForecast.length) {
      fetchCityWeather(selectedCity.lat, selectedCity.lon, selectedCity.city, selectedCity.state || '');
    }
  }, [selectedCity]);
