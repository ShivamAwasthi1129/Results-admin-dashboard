...
async function fetchLiveDisasters(): Promise<LiveDisaster[]> {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/live-disasters`, {
      cache: 'no-store',
    });
...
async function fetchWeatherData(): Promise<WeatherData[]> {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/weather?type=multi`, {
      cache: 'no-store',
    });
