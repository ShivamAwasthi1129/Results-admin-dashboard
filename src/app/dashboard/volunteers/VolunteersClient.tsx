...
export default function VolunteersClient({
  ...
  // Helper function to get full name from volunteer
  ...
  const handleRemoveDisasterFromVolunteer = async (volunteerId: string, disasterId: string) => {
    if (!confirm('Are you sure you want to remove this disaster assignment from the volunteer?')) return;
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        toast.success('Disaster assignment removed successfully!');
        // Refresh volunteers to update the UI
        ...
        toast.error(data.error || 'Failed to remove disaster assignment');
      }
    } catch (error) {
      console.error('Error removing disaster assignment:', error);
      toast.error('Failed to remove disaster assignment');
    }
  };
  ...
  const openEditModal = (volunteer: Volunteer) => {
    ...
    // Extract firstName and lastName from userId
    let firstName = volunteer.userId?.firstName || '';
    let lastName = volunteer.userId?.lastName || '';
    if (!firstName && !lastName && volunteer.userId?.name) {
...
{canManage && (
  <div className="w-full">
    ...
    >
                  ...
                  >
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        <Avatar name={getVolunteerName(volunteer)} size="md" src={volunteer.profileImage} />
                        <div>
                          <div className="flex items-center gap-1.5">
                            <span className="font-semibold text-[var(--text-primary)]">{getVolunteerName(volunteer)}</span>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium">{volunteer.assignedDisasters?.length || 0}</span>
                        {volunteer.assignedDisasters && volunteer.assignedDisasters.length > 0 && (
                          <div className="flex -space-x-1">
                            {volunteer.assignedDisasters.slice(0, 3).map((ad, idx) => (
                              <div
                                key={idx}
                                className="w-5 h-5 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white text-[10px] font-bold border border-[var(--bg-card)]"
                                title={ad.disaster?.title || 'Disaster'}
                              >
                                {ad.disaster?.title?.charAt(0) || 'D'}
                              </div>
                            ))}
                            {volunteer.assignedDisasters.length > 3 && (
                              <div className="w-5 h-5 rounded-full bg-[var(--bg-input)] flex items-center justify-center text-[10px] text-[var(--text-muted)] border border-[var(--bg-card)]">
                                +{volunteer.assignedDisasters.length - 3}
                              </div>
                            )}
                          </div>
                          ...
                          <>
                            <button
                              onClick={async (e) => {
                                ...
                                const now = new Date();
                                const hasActiveAssignments = volunteer.assignedDisasters?.some(
                                  (ad: any) => {
                                ...
                                const isOnMission = volunteer.availability === 'on_mission' || hasActiveAssignments;
                                
                                if (isOnMission) {
                                  toast.error('This volunteer is currently on mission and cannot be assigned to another disaster until their current assignment period ends');
                                  return;
                                }
                              ...
                              title={volunteer.availability === 'on_mission' ? 'Volunteer is on mission - cannot assign' : 'Assign Disaster'}
                              disabled={volunteer.availability === 'on_mission'}
                            >
                ...
                <div className="p-5 bg-gradient-to-br from-blue-500/5 to-blue-600/10 rounded-2xl border border-blue-500/20 backdrop-blur-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="grid grid-cols-2 gap-4">
                      <Input label="First Name *" value={formData.firstName} onChange={(e) => setFormData({ ...formData, firstName: e.target.value })} required icon={<UserIcon className="w-5 h-5" />} placeholder="John" />
                      <Input label="Last Name *" value={formData.lastName} onChange={(e) => setFormData({ ...formData, lastName: e.target.value })} required icon={<UserIcon className="w-5 h-5" />} placeholder="Smith" />
                    </div>
        ...
        {selectedVolunteer && (
          <div className="space-y-6">
            ...
            <div className="flex items-start gap-5 p-5 bg-gradient-to-r from-[var(--primary-500)]/10 to-[var(--primary-700)]/10 rounded-2xl">
              <Avatar name={getVolunteerName(selectedVolunteer)} size="xl" src={selectedVolunteer.profileImage} />
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="text-xl font-bold text-[var(--text-primary)]">{getVolunteerName(selectedVolunteer)}</h3>
            ...
            {/* Assigned Disasters */}
            {selectedVolunteer.assignedDisasters && selectedVolunteer.assignedDisasters.length > 0 && (
              <div>
                <h4 className="text-sm font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-3">Assigned Disasters</h4>
                <div className="space-y-2">
                  {selectedVolunteer.assignedDisasters.map((ad, idx) => (
                    <div key={idx} className="p-3 bg-[var(--bg-input)] border border-[var(--border-color)] rounded-lg">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1">
                          <p className="font-medium text-[var(--text-primary)]">{ad.disaster?.title || 'Unknown Disaster'}</p>
                          <p className="text-xs text-[var(--text-muted)] mt-1">
                            {ad.disaster?.type} • {ad.disaster?.severity}
                          </p>
                          <p className="text-xs text-[var(--text-muted)] mt-1">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <Badge
                            variant={ad.status === 'active' ? 'success' : ad.status === 'completed' ? 'primary' : 'secondary'}
                            size="sm"
                          >
                            {ad.status}
                          </Badge>
                          {canManage && (
                            <button
                              ...
                              title="Remove Disaster Assignment"
                            >
      ...
      {/* Assign Disaster Modal */}
      <Modal
        isOpen={showAssignDisasterModal}
        onClose={() => {
          setShowAssignDisasterModal(false);
          setSelectedVolunteerForDisaster(null);
          setDisasterAssignment({ disasterId: '', fromDate: '', toDate: '' });
        }}
        title="Assign Disaster to Volunteer"
        size="md"
      >
        {selectedVolunteerForDisaster && (
          <div className="space-y-6">
            <div className="p-4 bg-[var(--bg-input)] rounded-lg">
              <p className="text-sm text-[var(--text-muted)] mb-1">Volunteer</p>
              <p className="font-semibold text-[var(--text-primary)]">{getVolunteerName(selectedVolunteerForDisaster)}</p>
              <p className="text-xs text-[var(--text-muted)] mt-1">{selectedVolunteerForDisaster.volunteerId}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="date"
                  value={disasterAssignment.fromDate}
                  onChange={(e) => setDisasterAssignment({ ...disasterAssignment, fromDate: e.target.value })}
                  className="w-full px-4 py-2.5 bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-lg text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-purple-500"
              ...
              <div>
                ...
                <input
                  type="date"
                  value={disasterAssignment.toDate}
                  onChange={(e) => setDisasterAssignment({ ...disasterAssignment, toDate: e.target.value })}
                  className="w-full px-4 py-2.5 bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-lg text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-purple-500"
                  min={disasterAssignment.fromDate || new Date().toISOString().split('T')[0]}
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-[var(--border-color)]">
              ...
              <Button
                variant="gradient"
                onClick={async () => {
                  if (!disasterAssignment.disasterId || !disasterAssignment.fromDate || !disasterAssignment.toDate) {
                    toast.error('Please fill all required fields');
                    return;
                  }
                  if (new Date(disasterAssignment.toDate) < new Date(disasterAssignment.fromDate)) {
                  ...
                  try {
                    const response = await fetch(`/api/volunteers/${selectedVolunteerForDisaster._id}/assign-disaster`, {
                      ...
                      body: JSON.stringify({
                        disasterId: disasterAssignment.disasterId,
                        fromDate: disasterAssignment.fromDate,
                        toDate: disasterAssignment.toDate,
                      })
                    });
                    const data = await response.json();
                    if (data.success) {
                      toast.success('Disaster assigned successfully!');
                      setShowAssignDisasterModal(false);
                      setSelectedVolunteerForDisaster(null);
                      setDisasterAssignment({ disasterId: '', fromDate: '', toDate: '' });
                      fetchVolunteers();
                    } else {
                      toast.error(data.error || 'Failed to assign disaster');
                    }
                  } catch (error) {
                    toast.error('Failed to assign disaster');
                  }
                }}
                disabled={!disasterAssignment.disasterId || !disasterAssignment.fromDate || !disasterAssignment.toDate}
              >
                Assign Disaster
              </Button>
            </div>
