...
// GET - Get list of users whose location I can see
export async function GET(request: NextRequest) {
  try {
    ...
    const apiUrl = `${trackingApiUrl}/api/tracking/location/visible?userId=${userId}`;

    console.log(`[tracking/visible] Fetching from: ${apiUrl}`);
    ...
    if (!response.ok) {
      ...
      console.error(`[tracking/visible] API error: ${response.status} - ${errorText}`);
      return NextResponse.json(
        { success: false, data: [], error: `Failed to fetch visible locations: ${response.status}` },
