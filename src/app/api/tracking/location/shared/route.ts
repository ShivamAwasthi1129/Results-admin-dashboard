...
// GET - Get list of users who can see my location
export async function GET(request: NextRequest) {
  try {
    ...
    const apiUrl = `${trackingApiUrl}/api/tracking/location/shared?userId=${userId}`;

    console.log(`[tracking/shared] Fetching from: ${apiUrl}`);
    ...
    if (!response.ok) {
      ...
      console.error(`[tracking/shared] API error: ${response.status} - ${errorText}`);
      return NextResponse.json(
        { success: false, data: [], error: `Failed to fetch shared locations: ${response.status}` },
