...
// GET - Get current location of specific user
export async function GET(
  ...
  try {
    ...
    const apiUrl = `${trackingApiUrl}/api/tracking/location/current/${userId}?userId=${adminUserId}`;

    console.log(`[tracking/current/${userId}] Fetching from: ${apiUrl}`);
    ...
    if (!response.ok) {
      ...
      console.error(`[tracking/current/${userId}] API error: ${response.status} - ${errorText}`);
      return NextResponse.json(
