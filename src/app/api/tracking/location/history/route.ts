...
export async function GET(request: NextRequest) {
  try {
    ...
    let apiUrl = `${trackingApiUrl}/api/tracking/location/history?userId=${userId}&limit=${limit}`;
    
    if (startDate) apiUrl += `&startDate=${startDate}`;
    if (endDate) apiUrl += `&endDate=${endDate}`;

    console.log(`[tracking/history] Fetching from: ${apiUrl}`);
    ...
    if (!response.ok) {
      ...
      console.error(`[tracking/history] API error: ${response.status} - ${errorText}`);
      return NextResponse.json(
