...
// GET - Get all geofences for user
export async function GET(request: NextRequest) {
  try {
    ...
    const apiUrl = `${trackingApiUrl}/api/geofence?userId=${userId}`;

    console.log(`[geofence] Fetching from: ${apiUrl}`);
    ...
    if (!response.ok) {
      ...
      console.error(`[geofence] API error: ${response.status} - ${errorText}`);
      return NextResponse.json(
        { success: false, data: [], error: `Failed to fetch geofences: ${response.status}` },
