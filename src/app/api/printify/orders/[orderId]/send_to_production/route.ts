...
/**
 * POST /api/printify/orders/[orderId]/send_to_production?shop_id=...
 */
export async function POST(
  ...
  try {
    const url = `${PRINTIFY_BASE}/shops/${encodeURIComponent(shopId)}/orders/${encodeURIComponent(orderId)}/send_to_production.json`;
    ...
    console.error('[api/printify/orders/[orderId]/send_to_production]', err);
    return NextResponse.json(
      { success: false, error: 'Failed to send order to production' },
      { status: 500 }
    );
  }
}
